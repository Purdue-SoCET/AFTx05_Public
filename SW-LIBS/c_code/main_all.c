...
void Conv(uint8_t img[1][IMG_SIZE][1], uint8_t output_conv_arr[IMG_SIZE][N_FILTERS]);
...
uint8_t argMax(uint8_t quantized_input[1][N_CLASSES]);
int readFile(uint8_t img[1][IMG_SIZE][1], int index);
...
int main (void) {
	...
	for (int i = 0; i < num_imgs; i++) {
		lbl = readFile(img, i);
		Conv(img, output_conv);
		FullyConnectedDense(output_conv, output_dense);
		final_pred = argMax(output_dense);
		if (final_pred == lbl) {
			acc++;
		}
	}
	printf("ACC: %.2f", acc / num_imgs);
	return 0;
}
